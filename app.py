...
# CITY LAT/LON ANCHOR REGISTRY (For Dynamic Smart Anchoring)
CITY_ANCHORS = {
...
if not st.session_state.tour_guide:
    ...
    with col_f1:
        # THE THREE PRIMARY HIGH-LEVEL CORE CATEGORIES
    ...
    with col_f2:
        selected_city = st.selectbox("Select Current Moroccan City Location:", [
            "Casablanca", "Marrakesh", "Fes", "Tangier", "Rabat", "Agadir", "Chefchaouen"
        ])
    ...
    # DYNAMIC SMART ANCHORING: Shifts user's location instantly to match their current selected city!
    ...
    for place in TOUR_DATABASE:
        if place["cat"] == selected_cat and place["city"] == selected_city:
            # Check if sub-category filter matches
            is_sub_match = (selected_sub in ["All Culinary Spots", "All Monuments", "All Retail Options"]) or (place["sub"] == selected_sub)
            
            if is_sub_match:
                # Math formula calculates real proximity relative to the new active city anchor!
    ...
    # 5. SECURE ROUTING ENVIRONMENT & DIRECTION OUTPUT ENGINE
    if st.session_state.selected_destination:
        ...
        with c_nav1:
            ...
            # STAGE DIRECTIONS CONTAINER - ZERO TRADEMARK COMPLIANCE RISKS
        ...
        with c_nav2:
            # Renders a localized map zoomed right into the matching city parameters!
...
st.divider()
st.subheader("皮 Active Weather Wardrobe View")
